...
export default function DashboardPage() {
  ...
  const handleLogout = async () => {
    ...
    try {
      // Call the disconnect function which will now show a guidance toast and open MetaMask
      await disconnect()
      
      // Give the user a moment to see the toast notification before redirecting
      setTimeout(() => {
        router.push("/")
      }, 1000)
    } catch (error) {
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Connected to metMsk wallet</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
